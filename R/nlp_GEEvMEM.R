...
#' @keywords internal
.nlp2 <- function(data, methodvar, by, stats, target, top, linewidth=1, subtitle="", stextsize=8,
  ...
  gg2 <- gg2 +
    ...
    theme(axis.title.x=element_text(size=atextsize),
          axis.title.y=element_text(size=atextsize))
  ...
  for (i in seq_along(by)) {
    ...
    gg2 <- gg2 + #ggplot2::ggplot(data = data, mapping = ggplot2::aes(x = .scenario, y = est)) +
      ...
      theme(panel.grid.major.y = element_blank(), panel.grid.minor.y = element_blank(),
            ...
            legend.position = "bottom", plot.margin = unit(c(0,0.1,0.2,0.1), 'lines'), text = element_text(size = atextsize)) +
      scale_x_continuous(breaks = seq(1, 72, by = 2))
  }
