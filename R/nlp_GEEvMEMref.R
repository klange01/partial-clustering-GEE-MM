...
.nlp2ref <- function(data, methodvar, by, stats, target, reflow, refhigh, top, linewidth=1,
  ...
  if (!is.null(methodvar)) {
    ...
    gg <- ggplot2::ggplot(data, mapping = ggplot2::aes(x = .scenario, y = est, group = !!methodvar)) +
      ...
      ggplot2::geom_hline(yintercept = reflow, linetype = "dotted") +
      ggplot2::geom_hline(yintercept = refhigh, linetype = "dotted") +
  ...
  if(!is.null(pcol)){gg <- gg +
    scale_color_manual(name = "Analysis method", labels=mlab, values=pcol)
  }
  gg2 <- gg2 +
