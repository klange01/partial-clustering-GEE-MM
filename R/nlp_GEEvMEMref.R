...
# Includes y axis reference lines.
...
#' @keywords internal
.nlp2ref <- function(data, methodvar, by, stats, target, reflow, refhigh, top, linewidth=1, subtitle="", stextsize=8,
  ...
  # include a horizontal segment at end of step plot
  # duplicates the last two rows of data (one for each method) and makes them an additional scenario
  data = rbind(data, tail(data, 2))
  data[(nrow(data)-1):nrow(data), ".scenario"] = data[nrow(data)-2, ".scenario"] + 1
  ...
  if (!is.null(methodvar)) {
    ...
    gg <- ggplot2::ggplot(data, mapping = ggplot2::aes(x = .scenario, y = est, group = !!methodvar)) +
      ...
      ggplot2::geom_hline(yintercept = reflow, linetype = "dotted", col = "grey50", linewidth = linewidth-0.1) +
      ggplot2::geom_hline(yintercept = refhigh, linetype = "dotted", col = "grey50", linewidth = linewidth-0.1) +
  ...
  gg <- gg +
	...
	ggplot2::labs(x = NULL, y = ylab, subtitle = subtitle) + theme_bw() +
    theme(axis.line.x = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank(),
          ...
          plot.subtitle = element_text(size=stextsize)) +
